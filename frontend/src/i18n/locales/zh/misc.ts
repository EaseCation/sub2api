export default {
  ...
  payment: {
    ...
    errors: {
      ...
      ALIPAY_CONFIG_INVALID_AUTH_MODE: '支付宝接口加签方式必须为普通公钥或公钥证书。',
      ALIPAY_CONFIG_MISSING_CERT: '支付宝公钥证书模式缺少必填证书：{key}',
      ALIPAY_CONFIG_INVALID_CERT: '支付宝 {key} 无效，请在对应字段粘贴完整的 PEM 证书内容；根证书文件须包含 RSA 根证书。',
      ALIPAY_CONFIG_INVALID_PRIVATE_KEY: '支付宝应用私钥无效，请填写完整的 RSA PKCS#1 或 PKCS#8 私钥。',
      ALIPAY_CONFIG_CERT_KEY_MISMATCH: '应用公钥证书与应用私钥不匹配，请使用同一对应用密钥。',
